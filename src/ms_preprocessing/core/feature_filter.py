...
class FeatureFilter(BaseProcessor):
    ...
    4. Removes features with QC_ratio below threshold (default keeps legacy QC_ratio == 0 behavior)
    ...
    def _filter_features(
        ...
        # Rule 1: legacy QC ratio == 0
        qc_zero = (qc_ratio == 0)
        # Rule 2: user-defined minimum QC ratio
        qc_low = (
        ...
        # If QC ratio is low/zero and not protected, force delete
        keep_mask = np.where(qc_low & ~protected_mask, False, keep_mask)
