...
- Merge Mz and RT columns into FeatureID format (mz/RT)
...
class DataOrganizer(BaseProcessor):
    ...
    1. Merging Mz and RT columns into FeatureID (mz/RT format)
    ...
    def process(
        ...
        **kwargs,
        ...
        try:
            ...
            # Step 2: Merge Mz and RT into FeatureID
    ...
    def _merge_mz_rt(
        ...
        new_df = pd.DataFrame()
        new_df["Mz/RT"] = mz_rt_values

        # Optionally add Tolerance column (legacy; not required for ISTD marking)
        if include_tolerance_col:
            new_df["m/z Tolerance( ppm)/RT Tolerance"] = "na"

        # Add remaining columns (skip Mz and RT)
    ...
    def _parse_injection_sequence(self, file_path: Union[str, Path]) -> List[InjectionInfo]:
        ...
        try:
            ...
            for idx, table in enumerate(doc.tables):
                if len(table.rows) > 10:  # Must have many rows
                    header_cells = [cell.text.strip().lower() for cell in table.rows[0].cells]
                    # Look for "file name" column header
