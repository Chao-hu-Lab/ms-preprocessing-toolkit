...
@pytest.mark.parametrize(
    (
        ...
        "expected_high",
        "expected_low",
        "expected_ratio_rescue",
    ),
    [
        ("loose", 0.00, 0.20, 1.5, 0.3, 0.1, 2.0),
        ("default", 0.25, 0.33, 2.0, 0.8, 0.2, 3.0),
        ("strict", 0.50, 0.50, 3.0, 0.8, 0.2, 3.0),
    ],
)
def test_pipeline_profiles_bundle_fixed_step1_to_step3_with_named_step4(
    ...
    assert profile["step4"]["high_det_thresh"] == pytest.approx(expected_high)
    assert profile["step4"]["low_det_thresh"] == pytest.approx(expected_low)
    ...
    assert profile["step4"]["ratio_rescue_threshold"] == pytest.approx(expected_ratio_rescue)
